...
ll solve(vpii vec, ll ignore, ll totalCnt) {
    vi pref(totalCnt + 1, 0);


    for (auto i : vec) {
        if (ignore >= i.first && ignore <= i.second) {
            continue;
        }
...
void solve()
{
    ...
    ll ans = solve(vec, 1, totalCnt);
    ll ans1 = solve(vec, totalCnt - 1, totalCnt);
    ans = max(ans, ans1);
